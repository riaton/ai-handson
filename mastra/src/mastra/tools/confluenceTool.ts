...
function getAuthHandlers(): Record<string, string>{
...
async function callConfluenceAPI(endpoint: string, options: RequestInit = {}): Promise<any>{
...
export const confluenceGetPageTool = createTool({
    ...
    outputSchema: z.object({
        page: z.object({
            ...
            content: z.string().optional().describe("ページのコンテンツURL(HTML形式)"),
        }),
        error: z.string().optional().describe("エラーメッセージ"),
    }),
    execute: async({ context }) => {
